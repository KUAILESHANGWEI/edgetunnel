		...
		if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					const 订阅TOKEN = await MD5MD5(host + userID), 作为优选订阅生成器 = ['1', 'true'].includes(env.BEST_SUB) && url.searchParams.get('host') === 'example.com' && url.searchParams.get('uuid') === '00000000-0000-4000-8000-000000000000' && UA.toLowerCase().includes('tunnel (https://github.com/KUAILESHANGWEI/edge');
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							try {
								const response = await fetch(订阅转换URL, { headers: { 'User-Agent': 'Subconverter for ' + 订阅类型 + ' edge' + 'tunnel (https://github.com/KUAILESHANGWEI/edge' + 'tunnel)' } });
								if (response.ok) {
	...
	try {
		...
		const 添加规则集 = (type, code) => {
			...
			if (!ruleSet.some(item => item?.tag === tag)) {
				...
				ruleSet.push({ tag, type: 'remote', format: 'binary', url: `https://raw.githubusercontent.com/KUAILESHANGWEI/edgetunnel/main/assets/mirrors/SagerNet/sing-${type}/rule-set/${tag}.srs`, ...(legacyOptions?.download_detour ? { download_detour: legacyOptions.download_detour } : {}) });
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	const host = hostname, Ali_DoH = "https://dns.alidns.com/dns-query", ECH_SNI = "cloudflare-ech.com", 占位符 = '{{IP:PORT}}', 初始化开始时间 = performance.now(), 默认配置JSON = {
		...
		订阅转换配置: {
			...
			SUBCONFIG: "https://raw.githubusercontent.com/KUAILESHANGWEI/edgetunnel/main/assets/mirrors/ACL4SSR/Clash/config/ACL4SSR_Online_Mini_MultiMode_CF.ini",
			SUBEMOJI: false,
		},
		反代: {
			[_p]: "auto",
...
async function 生成随机IP(request, count = 16, 指定端口 = -1) {
	...
	const cidr_url = 运营商文件标识 === 'cf' ? 'https://raw.githubusercontent.com/KUAILESHANGWEI/edgetunnel/main/assets/mirrors/CF-CIDR/CF-CIDR.txt' : `https://raw.githubusercontent.com/KUAILESHANGWEI/edgetunnel/main/assets/mirrors/CF-CIDR/${运营商文件标识}.txt`;
...
async function 获取优选订阅生成器数据(优选订阅生成器HOST) {
	...
	try {
		const response = await fetch(优选订阅生成器URL, {
			headers: { 'User-Agent': 'v2rayN/edge' + 'tunnel (https://github.com/KUAILESHANGWEI/edge' + 'tunnel)' }
		});
